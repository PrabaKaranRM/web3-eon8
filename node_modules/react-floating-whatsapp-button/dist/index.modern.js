import { useState, useEffect, createElement } from 'react';

var styles = {"floatingWpp":"_34Oew","floatingWppButton":"_1zufg","floatingWppPopup":"_1jTF6","active":"_3sodH","floatingWppMessage":"_uThfQ","floatingWppHead":"_LDfCy","floatingWppInputMessage":"_22mst","floatingWppBtnSend":"_NeBrT"};

var FloatingWhatsApp = function FloatingWhatsApp(_ref) {
  var _ref$phone = _ref.phone,
      phone = _ref$phone === void 0 ? '1231231231' : _ref$phone,
      _ref$message = _ref.message,
      message = _ref$message === void 0 ? '' : _ref$message,
      _ref$size = _ref.size,
      size = _ref$size === void 0 ? '70px' : _ref$size,
      _ref$backgroundColor = _ref.backgroundColor,
      backgroundColor = _ref$backgroundColor === void 0 ? '#25D366' : _ref$backgroundColor,
      _ref$position = _ref.position,
      position = _ref$position === void 0 ? 'right' : _ref$position,
      _ref$popupMessage = _ref.popupMessage,
      popupMessage = _ref$popupMessage === void 0 ? 'Hello, how can we help you?' : _ref$popupMessage,
      _ref$showPopup = _ref.showPopup,
      showPopup = _ref$showPopup === void 0 ? false : _ref$showPopup,
      _ref$showOnIE = _ref.showOnIE,
      showOnIE = _ref$showOnIE === void 0 ? true : _ref$showOnIE,
      _ref$autoOpenTimeout = _ref.autoOpenTimeout,
      autoOpenTimeout = _ref$autoOpenTimeout === void 0 ? 2000 : _ref$autoOpenTimeout,
      _ref$headerColor = _ref.headerColor,
      headerColor = _ref$headerColor === void 0 ? '#128C7E' : _ref$headerColor,
      _ref$headerTitle = _ref.headerTitle,
      headerTitle = _ref$headerTitle === void 0 ? 'WhatsApp Chat' : _ref$headerTitle,
      _ref$buttonImage = _ref.buttonImage,
      buttonImage = _ref$buttonImage === void 0 ? 'data:image/png;base64,' : _ref$buttonImage,
      _ref$zIndex = _ref.zIndex,
      zIndex = _ref$zIndex === void 0 ? 0 : _ref$zIndex;

  var _React$useState = useState(showPopup),
      openPopup = _React$useState[0],
      setOpenPopup = _React$useState[1];

  var _React$useState2 = useState(message),
      messageInput = _React$useState2[0],
      setMessageInput = _React$useState2[1];

  var floatingWppButtonClicked = function floatingWppButtonClicked() {
    setOpenPopup(true);
  };

  useEffect(function () {
    setTimeout(function () {
      return setOpenPopup(true);
    }, autoOpenTimeout);
  }, []);

  var isMobile = function isMobile() {
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) return true;
    return false;
  };

  var isInternetExplorer = function isInternetExplorer() {
    var userAgent = navigator.userAgent;
    return userAgent && (userAgent.indexOf('MSIE') >= 0 || userAgent.match(/Trident.*rv:11\./));
  };

  var sendWhatsappMessage = function sendWhatsappMessage() {
    var apilink = 'http://';
    apilink += isMobile() ? 'api' : 'web';
    apilink += '.whatsapp.com/send?phone=' + phone + '&text=' + encodeURI(messageInput);
    window.open(apilink);
  };

  return createElement("div", {
    className: styles.floatingWpp,
    style: {
      zIndex: zIndex,
      right: position === 'right' ? '20px' : 'unset',
      left: position === 'right' ? 'auto' : 'unset',
      display: !isInternetExplorer() || showOnIE ? 'initial' : 'none'
    }
  }, createElement("div", {
    className: styles.floatingWppButton,
    style: {
      height: size,
      width: size,
      backgroundColor: backgroundColor
    },
    onClick: function onClick() {
      floatingWppButtonClicked();
    }
  }, createElement("img", {
    src: buttonImage
  })), createElement("div", {
    className: (openPopup ? styles.active : '') + " " + styles.floatingWppPopup,
    style: position === 'right' ? {
      right: 0
    } : {}
  }, createElement("div", {
    className: styles.floatingWppHead,
    style: {
      backgroundColor: headerColor
    }
  }, createElement("span", null, headerTitle), createElement("strong", {
    className: styles.close,
    onClick: function onClick() {
      return setOpenPopup(false);
    }
  }, "\xD7")), createElement("div", {
    className: styles.floatingWppMessage
  }, popupMessage), createElement("div", {
    className: styles.floatingWppInputMessage
  }, createElement("textarea", {
    value: messageInput,
    onChange: function onChange(e) {
      return setMessageInput(e.currentTarget.value);
    }
  }), createElement("div", {
    className: styles.floatingWppBtnSend,
    onClick: function onClick() {
      return sendWhatsappMessage();
    }
  }, createElement("img", {
    src: 'data:image/png;base64,',
    style: {
      height: 'inherit',
      width: 'inherit'
    }
  })))));
};

export { FloatingWhatsApp };
//# sourceMappingURL=index.modern.js.map
